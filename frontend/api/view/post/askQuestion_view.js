	...
	var askQuestion = Backbone.View.extend({
		...
		showAskForm: function(e){
			...
			if( self.currentUserId == false ){
				...
				//app.view.messageView.render('','Вы должны авторизироваться, для того что бы задать вопрос автору.');
				return;
			}
		...
		sendQuestion: function(){
			...
			$.post('/api/author/setQuestion',questionObj,function(data){
				if( data.error ){
					app.view.messageView.render('','Произошла ошибка при сохранении данных на сервере, попробуйте еще раз.');
					return;
				}
		...
		hidePreloader: function(){
			...
			$('.loaderConteiner',this.$el).remove();
		}
