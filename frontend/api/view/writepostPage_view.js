	...
	var writepostPageView = Backbone.View.extend({
		...
		initialize: function(){
			...
			$.get('/api/category/get_CategoryList',{},function(data){
				...
				self.questionCollection = new questionCollection(null,{ writePostModel: self.writepostModel, view: self	});
				self.listenTo(self.questionCollection, 'questionsReady',function(){ $('.questions_list .loader',self.$el).remove(); });
				self.render();
			},'json');					
