define([
	...
	var writepostPageView = Backbone.View.extend({
		...
		putPostData: function(){
			...
			if( postData.authorId !== app.userModel.attributes.userData.id && app.userModel.attributes.userData.role !== 4){
		...
		savePost: function(e){
			...
			if( !errors ){
				...
				//this.writepostModel.generateFullText();
		...
		deletePost: function(e){
			e.preventDefault();
			var self = this;
			app.view.messageView.confirm(
				config.getContent(41),
				config.getContent(42),
				function(confirm){
					if( confirm ){
						$('.buttons_area',self.$el).empty().append(config.loaderImg.big);
						self.writepostModel.deletePost();
					}
				}
			)
		}
