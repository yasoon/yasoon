  ...
  define('views/PostCategories',['views/PostCategoryView', 'backbone', 'mediator'], function(PostCategoryView) {
    ...
    return PostCategoriesList = (function(_super) {
      ...
      PostCategoriesList.prototype.checkedCategories = function() {
        ...
        categories = _.map(this.collection.where({
          'checked': true
        }), function(model) {
          return model.get('id');
        });
        block = categories.length > 2 ? true : false;
        this.$('input:checkbox:not(:checked)').prop('disabled', block);
        return categories;
      };

      PostCategoriesList.prototype.checkedCategoriesNames = function() {
        var namesList;
        namesList = _.map(this.collection.where({
          'checked': true
        }), function(model) {
